/* Shared deterministic circuit layout for recognition and the editor. */
(function circuitLayoutModule(root) {
  const renderer =
  ...
  const { getPins, transformPoint, componentLabelLayout, componentValue } = renderer;
  const { buildNets } = engine;
  const GRID = 20;
  const LIMIT = 100000;
  ...
  function normalizeCircuitLayout(document) {
    ...
    const wires = document.wires.map((wire) => {
      ...
      if (!candidates.length)
        throw new Error('无法在保留电路连接的前提下生成不穿过元件的正交导线。');
      candidates.sort(
        (first, second) =>
  ...
  const exported = { normalizeCircuitLayout };
  if (typeof module !== 'undefined' && module.exports) module.exports = exported;
  Object.assign(root, { FreeBbsCircuitLayout: exported });
