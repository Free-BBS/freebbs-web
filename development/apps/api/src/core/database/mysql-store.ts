...
const definitions = {
  ...
  collectionForms: {
    ...
    fields: [
      ...
      field('organizationId', 'organization_id'),
      field('currentDraftVersionId', 'current_draft_version_id'),
      field('publishedVersionId', 'published_version_id'),
      utcDateTimeField('opensAt', 'opens_at'),
      utcDateTimeField('closesAt', 'closes_at'),
      defaultedField('capacity', 'capacity', null),
    ],
    searchColumns: ['title', 'description', 'organization_id'],
  },
  collectionVersions: {
    table: 'collection_versions',
    conflictMessage: 'Collection version already exists',
    fields: [
      field('formId', 'form_id'),
      positiveIntegerField('version', 'version'),
      jsonField('schema', 'schema_json'),
      utcDateTimeField('publishedAt', 'published_at'),
    ],
    searchColumns: ['form_id'],
  },
  collectionResponses: {
    table: 'collection_responses',
    conflictMessage: 'Collection response already exists',
    fields: [
      field('formId', 'form_id'),
      field('versionId', 'version_id'),
      field('respondentUid', 'respondent_uid'),
      positiveIntegerField('attempt', 'attempt'),
  ...
  showcaseArticles: {
    ...
    fields: [
      ...
      field('body', 'body'),
      field('coverUrl', 'cover_url'),
      field('externalUrl', 'external_url'),
      field('organizationId', 'organization_id'),
      utcDateTimeField('publishedAt', 'published_at'),
    ],
    searchColumns: ['title', 'excerpt', 'body', 'organization_id'],
  },
  showcaseLikes: {
    table: 'showcase_likes',
    conflictMessage: 'Showcase like already exists',
    fields: [field('articleId', 'article_id'), field('userUid', 'user_uid')],
    searchColumns: ['article_id', 'user_uid'],
  },
  sportsTeams: {
    table: 'sports_teams',
    fields: [
