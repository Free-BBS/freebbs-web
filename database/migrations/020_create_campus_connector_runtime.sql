SET @freebbs_020_sql = (
    SELECT IF(
        COUNT(*) = 0,
        'ALTER TABLE notifications ADD COLUMN dedupe_key VARCHAR(128) NULL AFTER source_reference',
        'SELECT 1'
    )
...
PREPARE freebbs_020_statement FROM @freebbs_020_sql;
EXECUTE freebbs_020_statement;
DEALLOCATE PREPARE freebbs_020_statement;

SET @freebbs_020_sql = (
    SELECT IF(
        COUNT(*) = 0,
        'ALTER TABLE notifications ADD UNIQUE KEY uq_notifications_recipient_dedupe (recipient_user_id, dedupe_key)',
        'SELECT 1'
    )
    FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'notifications'
      AND INDEX_NAME = 'uq_notifications_recipient_dedupe'
      AND NON_UNIQUE = 0
);
PREPARE freebbs_020_statement FROM @freebbs_020_sql;
EXECUTE freebbs_020_statement;
...
SET @freebbs_020_sql = (
    SELECT IF(
        COUNT(*) = 0,
        'ALTER TABLE important_items ADD COLUMN action_url VARCHAR(512) NULL AFTER description',
        'SELECT 1'
    )
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'important_items'
