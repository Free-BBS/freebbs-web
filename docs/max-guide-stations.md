...
本文核对的是 `freebbs-web-onboarding` 独立工作区的现有页面和后端，不借用学习区分支的未合入功能。配置在 `public/max-guide-stations.js`，当前有 14 站、48 步；本地预览复用这些真实前端页面，而不是另外画一份示意网站。已发布的 `max-v2` 前 43 步保持原有索引，新站追加在末尾。
...
| 知识点     | 基础信息/实际应用概览；Markdown 正文与公式、图片、代码；知识标签；前后知识点；学习面板开关与面板内 Max/讨论 | 基础信息和应用只显示作者实际填写的内容。学习资源、学习反馈、继续学习、个人笔记、参与共建目前是预留接口；知识起源只有可折叠外壳，正文仍在开发                         |
| 讨论       | 版块与筛选；优先读一篇置顶帖；正文、回复与反应；独立发帖页入口；评论里的 `@Max`                             | 发帖编辑器位于 `/publish`，导览只介绍入口，不自动点击或跳转。`@Max` 在发表包含独立提及的评论/回复后才触发答疑，单独写在发帖正文不触发；导览不发表、不表态、不调用 AI |
...
| 随身手册   | 五项探索任务；当前功能介绍；未来计划；重看导览入口；完整导引一次性奖励                                      | 单纯访问任务不发奖；完整导引每账号一次 10 电元和 10 磁元，新老用户同享，不代表学习成绩；深入评测等 V2 想法仍待后续论证，不能描述为已开放                             |
| 发展端     | 当前建设状态与目标版本；规划中的同学服务和组织协作                                                          | 发展端仍在开发与联调；活动报名当前独立开放，上线后将整合进入发展端，尚未完成整合                                                                                     |
| 活动报名   | 当前独立入口；报名状态筛选；活动时间、名额、报名条件与回执查询说明                                          | 只浏览介绍，不填写或提交报名，不读取回执；无活动时说明流程并允许跳过                                                                                                 |
...
| 数学岛 / 在建岛                            | `.island-orbit-item[data-world-id="mathematics"]`、`[data-world-id="physics"]`                                                   | 打开领域概览；数学岛高亮期间临时隐藏 `#world-core`，离开后恢复；在建星球只说明状态                             |
...
| 知识工具与陪伴                             | `#knowledge-tools`、`#knowledge-chat-toggle`、`#knowledge-chat-close`、`#knowledge-chat-panel`、`#knowledge-chat-tab-discussion` | 同一步先展示右侧开关，再打开并介绍学习面板；可收起回到开关，继续按钮收起面板后前进；不点学习标签或发送 AI 提问 |
| `/discussion?board=<slug>&post=<id>`       | `.discussion-feed-toolbar`、`#discussion-post-list`、`[data-action="open-post"]`                                                 | 读列表与具体帖子；没有帖子时允许跳过                                                                           |
| 帖子详情与发帖入口                         | `#discussion-detail`、`#discussion-comment-form`、`[data-action="close-detail"]`、`#discussion-create-toggle`                    | 详情可只读展开/关闭；发帖步骤只高亮入口，下一步继续导览，不自动打开 `/publish` 或提交表单                      |
| `/publish?board=<slug>`（预览可手动访问）  | `#discussion-compose-form`                                                                                                       | 独立编辑页复用真实前端；预览拒绝发布写入，导览不进入或填写该页                                                 |
...
| `/development`                             | `.development-release`、`.development-card-student .development-card-heading`                                                    | 仅讲解建设状态和未来整合方向，不把规划描述成已开放功能                                                         |
| `/surveys`                                 | `.activity-hero-links`、`.activity-filters`、`.activity-grid .card:first-child .activity-meta`                                   | 只读介绍；无卡片或列表未加载时回退到固定入口说明，不打开或提交表单                                             |
...
- 每一步含 `id/station/route/target/label/title/body/caption`。`action` 是明确审核过的只读 `click` 或 `link`，跨步骤前应先存进度再执行；同一步的 `reveal` 展开与收起只改变当前讲解视图，不推进保存的步骤索引。
...
- `world-mathematics` 使用 `focus: {hide: '#world-core'}`，仅在数学岛聚光期间临时隐藏中央主岛，避免透明岛屿素材透出主岛标签；切步、暂停或关闭导览时恢复，不删除主岛或修改其持久状态。
- `knowledge-companions` 保留原有步骤 ID 和索引。先通过 `#knowledge-chat-close` 确保面板收起，高亮 `#knowledge-chat-toggle`；打开后由 `reveal` 高亮 `#knowledge-chat-panel` 并介绍两个标签。点击面板关闭按钮或右侧开关会回到同一步的入口讲解；点击「收起面板，继续导览」才保存下一步并继续。不自动切换课程讨论、发送问题或发表评论。
- `discussion-composer` 只高亮 `#discussion-create-toggle`。现有按钮实际进入 `/publish?board=<slug>`，所以该步骤不再自动点击按钮或等待讨论列表中不存在的编辑表单；用户可在暂停或结束导览后自行进入发帖页。
...
已发布的 `guide-depth-2026-09` 短游固定使用以下 9 个 ID，不能覆盖或重排；个人主页使用当前登录账号的真实 UID：
...
该发布记录已经发布，后续更新必须追加新的 release，而不是重排其 `stepIds`。`profile-ranch` 聚焦 `#public-profile-ranch .ranch-scene` 的 Max 与茅草屋，`profile-wool` 聚焦 `.ranch-wool-stages` 的剪取/摩擦区域，避免整块超长牧场遮挡视口。未拥有 Max 时回退到 `#public-profile-ranch` 解释前置条件；每累计5次有效喂养长1份羊毛，按北京时间每天最多剪1份，其余待剪量保留，次日零点恢复机会。当天再剪提示「Max 被薅秃了，明天再来吧。」；剪下后使用7磁元购买的永久橡胶棒摩擦，每份换2电元。导览只讲解，所有操作仍需用户自己触发。

新发布记录 `guide-community-2026-09` 使用 5 步：`development-status`、`development-activities-plan`、`activities-entry`、`activities-browse`、`activities-receipt`。新站也追加到完整导览末尾。已有完整导览和历史短游的完成状态、位置与任务收据保留，新版本通过既有一次性更新提示进入。新步骤没有 `action` 或 `prepare`，只读取公开区域；聚光复用既有明暗主题与定位引擎，圈定状态条、局部标题、筛选条或首张卡片的信息，不圈整页。
...
- `/publish?board=<slug>` 复用真实独立发帖页及 `publish.js`、`publish.css`，便于用户手动查看入口是否连通；本地预览仍拒绝发帖写入，不把编辑页可打开描述为已经允许发布。
- 导览进度按 version 存入 Map，并直接复用后端 `createOnboardingService`：`max-v2`、历史 `max-v1`、`guide-depth-2026-09` 和 `guide-community-2026-09` 各自保留进度。新版基础导览只继承历史任务收据，不继承 seen/status/step；新版短游没有新手任务。读取进度本身不把欢迎记为已读。
- 本地预览的活动列表保持真实空态，没有伪造活动或开放报名写入；新报名导览可核查筛选入口和无卡片时的流程说明。
...
自动检查另覆盖已发布前 43 步索引与旧 release 的固定顺序、独立版本邀请、新短游从发展端跨页到活动报名与完成时保留旧进度、活动步骤无表单动作。

人工浏览器还需按最新 48 步逐项检查：跨页后继续、返回上一项、直接跳站、刷新后 prepare、无内容时跳过、原生弹窗 top layer、明暗主题及窄屏的聚光边界；确认不会自动发送 AI 或产生资产、资料、密码和报名变化。本文件不把旧版浏览器验证当作新版 48 步已验收，也不宣称未经浏览器核查的视口兼容性。
