...
自主执行的响应额外包含 `done`：有操作时为 `false`，完成回答且无操作时为 `true`。模型可以在 `circuit-actions` 块中使用 `{"actions":[],"done":true}` 结束，也可直接给出没有操作块的最终回答。无效批次保留 `actionWarning` 且 `done:false`，前端将校验错误送回下一轮；不会将无效操作误报为完成。只读上下文允许解释、高亮、查看和仿真，禁止修改草稿。
