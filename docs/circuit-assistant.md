...
服务端区分 180 秒没有有效生成内容和 300 秒单步总时限。只有实际增量或完成事件重置模型空闲计时，心跳不会让停滞调用无限挂起。浏览器使用 60 秒网络空闲和 300 秒请求总时限；Agent 每轮模型决策与动作执行合计最多 5 分钟，每次进入下一轮重新计时，任务没有 10 分钟总时限；停止、页面离开和断开连接会取消上游并释放流。SSE 提前展示进展，不保证模型本身更快；每轮只要求简短说明并合并可一起执行的操作，保留原有波形采样和极值精度。
...
`public/circuit-agent.js` 负责有限循环、停止、超时、快照版本检查及执行观察。每个任务最多 12 次模型决策，每轮决策与动作执行合计最多 5 分钟，没有任务总时长限制；连续 3 次失败、重复没有进展的操作或手动改动画布时结束。停止会取消请求及关联仿真，迟到的响应不能继续执行。侧栏逐步显示操作状态，收起侧栏仍可停止。每轮的修改由编辑器保存一个整体撤销点，结束后可恢复运行前状态；后续手动编辑后，旧撤销点不能覆盖新修改。

可执行工具包括元件参数、分析方式、元件增删和连线、位置与变换、高亮与仿真，以及 `set_plot`（公式、通道、X–T/X–Y、相位和坐标范围）、`set_annotation` / `delete_annotation`。标记的可选 `marker` 只接受 `point`、`vertical`、`horizontal`；新标记省略时显示为点，更新已有标记时省略则保留原有样式。点/线位于图内，注释与实际读数在图外图例；横线取实际采样点的 y，竖线取图像实际 x，仍通过原始采样坐标 `at` 定位，不允许自造数值。图像操作沿用真实采样与安全数学表达式校验；新数学曲线的极值须在执行后的下一轮读取，不能凭空生成。
...
电路 Max 和“问问 Max”接收独立的思考事件，在可折叠区域中按纯文本实时显示模型返回的 `reasoning_content`。思考内容不进入回答 Markdown、会话历史或电路操作校验；只有完整结果可以产生操作。并行调用的思考按 ID 分开显示，正文完成后自动折叠，停止后保留已收到的片段。

Agent 请求通过 `reasoning_stream: true` 启用新协议；普通聊天的最终事件保留原有导航和 RAG 元数据。电路后端将思考转为独立 `reasoning` SSE 事件，并据实际思考增量刷新模型空闲计时，心跳不刷新模型空闲计时。
