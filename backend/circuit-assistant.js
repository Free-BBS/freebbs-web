...
function buildCircuitAssistantPayload(input) {
  ...
  const instructions = [
    ...
    '{"type":"set_annotation","annotation":{"id":"A1","traceId":"V:R1","at":0.001,"text":"采样峰值","marker":"point","mode":"xt","axis":"value","xTraceId":null,"analysisKey":"transient"}}；新增标记或按相同 id 更新已有标记；每个电路最多 32 个。text 可为空，最多 160 字符；id 字母开头，仅字母数字下划线连字符，最多 40 字符。',
    'marker 可选 point（点，旧标记默认）、vertical（竖线）或 horizontal（横线）。图内只画对应点/线，注释和真实读数显示在图外图例，不遮挡曲线。事件时刻或负载拐点适合 vertical；采样值参考线适合 horizontal；峰值或单个位置适合 point。三种样式都以 at 指定的真实采样点定位：横线使用该点真实 y，竖线使用图像实际 x，不能填自造 y 值。更新已有标记时保留原有 marker，除非用户要求修改样式。',
